...
plot_pdf_fpath = os.path.join(output_dir, "bimodal_log10_python_vs_asv_by_transformation.pdf")
# --------------------------------------------------------------------------
	...
	for ds2 in comp_ds:
		if (ds1 != ds2):
			...
			for project in projects:
				...
				pval_fpath = os.path.join(op_dir, "tables", f"{project}_bimodal_pValuesUnivariate_sequenceVmetadata.csv")
				my_table = pd.read_csv(pval_fpath, sep=',', header=0)
				# print(my_table)
				for feat, ave in zip(list(ds2_table["metadata"].values) ,list(means)):
					ds2_score[feat] = ave
					if ( feat in my_table["meta_name"].values):
			...
			same_keys = set(same_keys.intersection(set(pvals.keys())))
