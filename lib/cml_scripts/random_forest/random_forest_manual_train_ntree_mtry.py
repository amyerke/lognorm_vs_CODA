...
parser.add_argument("-n", "--n_tree", default=100, type=int,
                  help="Number of estimators in the random forest classifier.")
...
main_output_label = f"sklrn_randmforst_manual_{train_percent}train_ntree{options.n_tree}_m_try{mtry_str}"
		...
		for name, table_info, color in tables:
			...
			for i in range(num_iterations):
				...
				if is_string_dtype(respns_var) == True:
					clf = RandomForestClassifier(max_depth=2, random_state=0, n_estimators=options.n_tree, max_features=options.m_try)
