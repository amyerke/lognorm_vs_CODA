...
parser.add_argument("-n", "--n_tree_step", default=100, type=int,
...
main_output_label = f"sklrn_randmforst_manual_{train_percent}train_variable_ntree_R_mtry"
...
#info for scatter
scatter_pdf_fpath = os.path.join(output_dir, "graphics", f"{main_output_label}.pdf")
time_dic = {}
...
with open(result_fpath, "w+") as fl:
	...
	fl.write("\n")
	for nt in range(options.n_tree_min, options.n_tree_max, options.n_tree_step):
			...
			for name, table_info, color in tables:
				...
				for i in range(num_iterations):
					...
					if is_string_dtype(respns_var) == True:
						clf = RandomForestClassifier(max_depth=2, random_state=0, n_estimators=nt, min_samples_split=math.ceil(len(set(pred_train))**(1/2)))
				...
				print(final_acc)
				msg = f"{name},{m_c},{color},{nt}, {final_acc}\n"
				print(msg)
				fl.write(msg)
...
pdf = matplotlib.backends.backend_pdf.PdfPages(scatter_pdf_fpath)
